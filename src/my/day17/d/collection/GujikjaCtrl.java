...
public class GujikjaCtrl extends Controller implements InterGujikjaCtrl {
	...
	public void showAllGujikja_info(List<Member> mbrList) {
		...
		if(count_gu == 0) {
		   // mbrArr 배열에 저장되어진 객체정보 중 Gujikja 객체가 관리자(admin)만 저장된 경우
		   System.out.println(">> 등록되어진 구직자가 아무도 없습니다. << \n");	
			...
			for(int i=0; i<mbrList.size(); i++) {
				...
				if( mbrList.get(i) instanceof Gujikja &&
				    !"admin".equals(mbrList.get(i).getId()) ) {
					((Gujikja)mbrList.get(i)).viewInfo();  //방법이 두가지 구직자로 캐스팅하든지 부모클래스에서 뷰인포를 추상메소드로 만들면된다
				}
				
			}// end of for--------------------------------
		}	
		
	}// end of public void showAllGujikja_info(List<Member> mbrList)--------------- 
	...
	} // end of public void searchAgeLineGender(List<Member> mbrList, Scanner sc)---------------
