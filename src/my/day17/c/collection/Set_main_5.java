...
*/
	public static void main(String[] args) {
      ...
      mbr_suji = mbr_chaew; // mbr_chaew 가 참조하고 있던 메모리 주소를 mbr_suji 에게 주었으므로  mbr_suji 은 new Member("chaew", "abcd1234$", "차은우"); 의 메모리 주소를 참조한다. 
                   // mbr2 는 new Member("chaew", "abcd1234$", "차은우"); 의 메모리 주소를 참조한다. 
      				//차은우의 메모리주소를 수지한테 넘겨주어 .
      ...
      while(it.hasNext()) {
          ...
          // 하나를 끄집어내서next 존재하는지를 알아보는 것이다.
          // 존재하면 true, 존재하지 않으면 false.
    	  
    	  Member mbr = it.next();  //Member mbr = 안써도됨
      ...
      System.out.println("\n>>퀴즈4<<\n");
      // 4. mbrHashSet 에 저장되어진 회원(Member)들 중에 id가 "leess" 인 회원을 삭제하세요.
      it = mbrHashSet.iterator();
      while(it.hasNext()) {
      ...
      // 2. Member 클래스의 객체 9개를 생성하여 mbrLinkedHashSet 에 저장한다.
      ...
      mbr_suji_2 = mbr_chaew_2; // mbr_chaew 가 참조하고 있던 메모리 주소를 mbr_suji 에게 주었으므로  mbr_suji 은 new Member("chaew", "abcd1234$", "차은우"); 의 메모리 주소를 참조한다. 
                   // mbr2 는 new Member("chaew", "abcd1234$", "차은우"); 의 메모리 주소를 참조한다. 
      				//차은우의 메모리주소를 수지한테 넘겨주어 .
      ...
      while(it2.hasNext()) {
    	  ...
    	  Member mbr = it2.next();  //Member mbr = 안써도됨
    	  // 실제로 Member를 it 에서  끄집어낸다.
          // 그러면 끄집어 낸 Member는 더이상 it 에는 남아있지 않게 된다.
