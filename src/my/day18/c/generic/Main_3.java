...
public class Main_3 {
	...
	public static void main(String[] args) {
		...
		///////////////////////////
		 ...
		 emp.register(new Employee("wonderwoman","0070","원더우먼","과장"));
		...
		//box.register_anything(Box_Anything<?> item);
		...
		////////////////////////////////////////////
		System.out.println("\n~~~ 와일드 카드의 하한제한 하기. T와 그 조상클래스들만 사용가능한 경우 ~~~~\n");
...
//		box.register_executive(box_plain);  //오류발생! 안들어간다 안된다!
		
		/*
		 box.register_executive(Box_Anything<? super Employee_Child_Executive> item)이므로
		 파라미터가 Employee_Child_Executive을 포함한 Employee_Child_Executive의 조상클래스들만 
		...
		////////////////////////////////////////////////
		System.out.println("\n~~~ 와일드 카드의 상한제한 하기. T와 그 자손클래스들만 사용가능한 경우 ~~~~\n");
...
//	  	box.register_plain(box_emp); //오류발생 맥시멈이 아님.
		/*
		 
		 resister_plain(Box_Anything<? extends Employee_Child_Plain> item)이므로
		 파라미터가 Employee_Child_Plain 를 포함한 자식클래스 까지만 입력이 가능한데
		 box_emp는 Box_Anything<Employee> 인데 이것은 Employee_Child_Plain의 부모클래스 이므로
		 상한제한에 걸려서 입력이 불가하다.
		...
		for(Box_Anything<? extends Employee_Child_Plain> b:child_plain_list ) {
			...
			List<Employee_Child_Plain> list = (List<Employee_Child_Plain>)b.getList();  //casting
