...
public class StringMain {
	...
	public static void main(String[] args) {
		...
		//내가 푼 것
		for(int i=0;i<pathFileNameArr.length;i++) {
			char[] pathArr = pathFileNameArr[i].toCharArray();
		    result = "";
...
//			System.out.println(strReverse);
		...
		/* 나혼자
		 * for(int i=0;i<money.length()+1;i++) { result =""; int idx =
		 * money.indexOf(",");
		 * 
...
//			"20,00,000".substring(0,1)  ==> "2"
//			money.substirng(commaIndex+1)  ==>  "000,000"
			// "2" + "000,000" ==> "2000,000"
			//"2000000"
		...
		DecimalFormat df = new DecimalFormat("#,###");//세자리마다 콤마를 찍어준다.
...
//		   if(strArr[i].equals("korea"))  //NullPointerException
		   if("korea".equals(strArr[i]))  //null이 아닌걸 먼저 써줘야함 그담엔 null이 와도됨
...
//	   for(int i=0;i<strArr.length;i++) {
////		   if(strArr[i].equalsIgnoreCase("korea"))  //NullPointerException
//		   if("KOREA".equalsIgnoreCase(strArr[i]))  //null이 아닌걸 앞에 써줘야함
//			   System.out.println(strArr[i]);
//	   }//end of for -------
	   ...
	   for(String s : strArr) {
		   ...
		   if( s != null && s.toLowerCase().indexOf(search) != -1) {
			   //s.toLowerCase()은 "korea","seul"," korea seoul","korea 대한민국 ", "서울 korea 만세"으로 되어진 상태이다.
