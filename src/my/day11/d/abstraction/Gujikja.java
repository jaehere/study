...
public class Gujikja {
	...
	// === 객체지향프로그래밍(Object Oriented Programming) 이란? ===//  필드 메소드 클래스
	//어떤 프로그램이냐에 따라 그 목적에 맞도록 필요한 데이터(필드)를 추상화(필요한 것만 추출하는 것, 반대가 정밀화)시키고,
	//또한 그 목적에 맞는 기능(메소드)만 부여한 "클래스"를 생성하여 프로그램하는 것이다.
	
	// === 객체지향프로그래밍(Object Oriented Programming)의 특징 ===//
	/*
	 	1. 상속성 (Inheritance) : 클래스의 재사용과 소스 코드의 중복제거를 목적으로 하는 기술  //중복제거
	 	2. 추상화 (Abstraction) : 프로그램이 필요로 하는 실제 데이터들을 모델링하는 것   //필요한것만 뽑아오는 것 예로 들기 ex. 쇼핑몰 , 병원 비교
	 	3. 캡슐화 (EnCapsulation == 은닉화) : 객체지향의 가장 중요한 데이터 보호 기술  //접근제어자 public protected default private  
	 	4. 다형성 (Polymorphism) : 상속을 이용하여 여러 클래스 타입을 하나의 클래스 타입으로 다루는 기술  // 부모클래스로 받겠다 . 저장Member 재정의 overriding 
	 	...
	 	다양한 객체들을 각각 하나하나 추상화 시켜서 클래스로 옮기는 것을 객체지향 프로그래밍(OOP)라고 한다.
