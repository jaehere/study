	...
	public static void main(String[] args) {
		...
		*/
			...
			for(int i=0;i<pointArr.length;i++) { // 행
				for(int j=0;j<pointArr[i].length;j++) { //열  그 행에 있는 열의 개수만큼 돌려라
...
//			int[][] jumsuArr = new int[3][]; //몇행인지는 꼭 넣어줘야 한다
							//  국어 영어 수학
...
*/	
			...
			/* 내가 풀다만거
			 * double[] subjectAvgArr = new double[jumsuArr[0].length]; //각 과목별 총점을 저장시켜두는
			 * 곳이다. for(int i=0; i<jumsuArr[0].length;i++) {
			 * 
			 * double avg=0; for(int j=0 ; j<jumsuArr.length;j++) { avg +=
			 * Math.round((double)jumsuArr[j][i]/jumsuArr.length*10)/10;
			 * 
			 * 
			 * }//end of for---------
			 * 
			 * subjectAvgArr[i] = avg; }//end of for----------
			 * 
			 * for(double avg : subjectAvgArr) { System.out.print(avg+" "); }
			 */
